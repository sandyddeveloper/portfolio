      ...
      <div className={`rounded-2xl border shadow-sm backdrop-blur-xl transition-all overflow-hidden ${
        ...
        <div className={`flex flex-wrap items-center justify-between gap-3 border-b px-6 py-3.5 ${
          ...
          <div className="flex items-center gap-3 text-xs font-mono">
            ...
            <span className={`font-bold ${theme === 'dark' ? 'text-purple-300' : 'text-purple-900'}`}>
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-purple-600 border border-purple-500 px-4 py-2 text-xs font-bold text-white hover:bg-purple-700 transition-all cursor-pointer shadow-md shadow-purple-500/20 disabled:opacity-50"
          >
            {isSimulating ? (
              <>
        ...
        <div className="p-6 relative space-y-6">
          ...
          {activePackets && (
            <div className="hidden md:block absolute top-16 left-12 right-12 h-0.5 bg-purple-500/20 z-0">
              <motion.div
            ...
            {nodes.map((node) => (
              <div
                ...
                className={`flex flex-col justify-between rounded-xl border p-4 transition-all cursor-pointer ${
                  node.enabled
                    ? theme === 'dark'
                      ? 'border-purple-900/40 bg-slate-900/80 hover:border-purple-500/40'
                      : 'border-purple-200 bg-purple-50/50 text-slate-950 font-bold hover:border-purple-400'
                    : 'border-slate-300/40 bg-slate-100/40 opacity-40'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className={`text-[10px] font-bold font-mono px-2 py-0.5 rounded ${
                      node.enabled ? 'bg-purple-100 text-purple-800 border border-purple-300' : 'bg-slate-200 text-slate-600'
                    }`}>
                      {node.enabled ? 'ACTIVE' : 'OFFLINE'}
                    </span>
                    <span className="text-[10px] font-mono text-emerald-600 font-bold">{node.loadMs}ms</span>
                  </div>

                  <h3 className={`mt-3 text-xs font-extrabold ${theme === 'dark' ? 'text-white' : 'text-slate-950'}`}>
                    {node.name}
                  </h3>
                  <p className={`text-[10px] font-mono mt-0.5 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>{node.type}</p>
                </div>
