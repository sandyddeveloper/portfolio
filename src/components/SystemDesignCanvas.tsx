  ...
  return (
    <section id="system-design" className="space-y-6 scroll-mt-24">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`rounded-md px-2 py-0.5 text-[10px] font-bold font-mono ${
              theme === 'dark' ? 'bg-umber border border-cedar text-silver' : 'bg-blue-500/10 border border-blue-500/30 text-blue-400'
            }`}>
        ...
        <p className={`max-w-md text-xs leading-relaxed ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
      ...
      <div className={`rounded-2xl border shadow-sm backdrop-blur-xl transition-all overflow-hidden ${
        ...
        <div className={`flex flex-wrap items-center justify-between gap-3 border-b px-6 py-3.5 ${
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-xl border px-4 py-2 text-xs font-bold transition-all cursor-pointer disabled:opacity-50 ${
              theme === 'dark'
        ...
        <div className="p-6 relative space-y-6">
          ...
          {activePackets && (
            <div className={`hidden md:block absolute top-16 left-12 right-12 h-0.5 z-0 ${theme === 'dark' ? 'bg-amber-800/40' : 'bg-cyan-500/20'}`}>
              <motion.div
                className={`h-full w-24 ${theme === 'dark' ? 'bg-gradient-to-r from-transparent via-amber-200 to-transparent' : 'bg-gradient-to-r from-transparent via-cyan-400 to-transparent'}`}
                animate={{ x: ['0%', '400%'] }}
            ...
            {nodes.map((node) => (
              <div
                ...
                className={`flex flex-col justify-between rounded-xl border p-4 transition-all cursor-pointer ${
                  node.enabled
                    ? theme === 'dark'
                      ? 'border-divider bg-darkBg text-silver hover:border-cedar hover:bg-umber/40 shadow-md'
                      : 'border-slate-200 bg-slate-50 hover:border-cyan-500/40'
                    : theme === 'dark' ? 'border-divider/30 bg-darkBg/30 text-slate-500 opacity-40' : 'border-slate-800/40 bg-slate-950/40 opacity-40'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className={`text-[10px] font-bold font-mono px-2 py-0.5 rounded ${
                      node.enabled
                        ? theme === 'dark' ? 'bg-umber border border-cedar text-silver' : 'bg-cyan-500/10 text-cyan-400'
                  ...
                  <h3 className={`mt-3 text-xs font-bold ${theme === 'dark' ? 'text-silver' : 'text-slate-900'}`}>
                    {node.name}
                  </h3>
                  <p className={`text-[10px] font-mono mt-0.5 ${theme === 'dark' ? 'text-amber-200/80' : 'text-slate-400'}`}>{node.type}</p>
                </div>

                <div className={`mt-4 pt-2 border-t flex items-center justify-between text-[10px] font-mono ${
                  theme === 'dark' ? 'border-divider text-slate-400' : 'border-slate-800/60 text-slate-500'
                }`}>
          ...
          <div className={`rounded-xl border p-4 font-mono text-xs flex items-center justify-between ${
            theme === 'dark' ? 'border-divider bg-darkBg text-silver' : 'border-slate-800 bg-slate-950 text-slate-300'
          }`}>
            <div className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-emerald-400 font-bold">Topology Status: Optimal</span>
              <span className="text-slate-500">•</span>
              <span className={theme === 'dark' ? 'text-slate-300' : 'text-slate-400'}>Zero bottle-necks detected</span>
            </div>
            <span className="text-[11px] text-slate-400">Auto-Balancing Engine</span>
          </div>
        </div>
      </div>
    </section>
